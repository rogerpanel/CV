...
Continuous-time modeling of temporal graphs addresses fundamental limitations of discrete-time approaches requiring fixed sampling intervals that trade off between temporal resolution and computational efficiency. We formulate graph evolution through continuous-time dynamics enabling principled handling of irregular event occurrences across multiple time scales.
...
where $\Delta t_i = t_{i+1} - t_i$ represents inter-arrival times, burstiness quantifies temporal clustering through autocorrelation, and entropy measures predictability of size distributions.
...
Microservices architectures exhibit hierarchical structure where security behaviors manifest at multiple granularities: service-level interactions between components, trace-level request flows spanning services, and node-level host communications. Single-granularity representations fail to capture cross-level dependencies enabling sophisticated attacks. We develop multi-granularity graph embeddings jointly modeling security across abstraction levels.
...
Federated learning enables collaborative threat detection across organizations without centralizing sensitive security data. However, standard federated optimization assumes non-adversarial participants and homogeneous data distributions. We develop Byzantine-robust federated learning integrated with large language models for zero-shot detection and differentially private optimal transport for distribution alignment.
...
Zero-shot detection of novel API attacks leverages large language models understanding attack semantics. Given API request sequence $\mathbf{r} = (r_1, \ldots, r_n)$ with endpoint paths, parameters, and response codes, we construct natural language prompt:
...
Post-quantum cryptographic protocols introduce distinct traffic patterns and attack surfaces requiring specialized detection capabilities. We develop feature extraction and modeling techniques for lattice-based, code-based, and hash-based post-quantum schemes.
...
State space models provide efficient temporal sequence modeling through linear differential equations, but their adversarial robustness under poisoning attacks requires formal analysis. We integrate Mamba's selective scan mechanism with progressive adversarial robustness distillation and PAC-Bayesian certification.
...
Transformers achieve state-of-the-art performance on sequence modeling but lack principled uncertainty quantification and adversarial robustness. We develop variational Bayesian transformers with stochastic adversarial training providing calibrated uncertainty estimates and certified robustness.
...
Adaptive adversaries observe deployed detection mechanisms and craft evasion strategies. Game-theoretic modeling characterizes equilibrium strategies where neither defender nor attacker benefits from unilateral deviation.
