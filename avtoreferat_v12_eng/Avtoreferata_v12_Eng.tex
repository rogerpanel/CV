...
\begin{enumerate}
\item Carry out a systematic review of the approaches used in
modern Hybrid~IDPS and identify their disadvantages and
vulnerabilities across seven application sectors.

\item Perform a review of methods to improve robustness against
adversarial attacks and to ensure computational efficiency,
together with a comparative analysis of current AI-augmented
commercial products; formulate uniform requirements for the
developed models and algorithms.

\item Formalise the mathematical model of the Hybrid~IDPS as the
tuple $\mathcal{H} = (\mathcal{V}_t, \mathcal{E}_t, X_t, A_t,
f_\theta, g_\psi, \pi)$ and the six cross-cutting properties
P1--P6 as invariant conditions, organising them into a
$3\times4$ condition--requirement matrix; explicitly pose the
multi-criterion problem of improving robustness, accuracy,
efficiency, and privacy.

\item Develop algorithms and models M1--M7 that improve the
robustness and efficiency of the Hybrid~IDPS in distributed and
non-stationary conditions: continuous-time graph dynamics
CT-TGNN/SDE-TGNN~(M1); federated optimisation FedLLM-API with
differential privacy~(M2); multi-level temporal embeddings
TripleE-TGNN~(M3); selective state-space model MambaShield~(M4);
uncertainty-calibrated inference Stochastic Transformer/UC-HGP
(M5--M6); and game-theoretic certification FedGTD~(M7).

\item Develop software implementing the proposed models and
algorithms: formulate the functional requirements, use cases,
architecture, and program realisation as the production-grade
platform \textbf{RobustIDPS.ai} (DOI~10.5281/zenodo.19129512;
17~neural-network models in 10~functional groups, 51~pages of a
Progressive Web Application).

\item Conduct experimental research of the developed algorithms
and models on benchmark datasets: 6~network-traffic benchmarks
totalling over 84.2~million labelled samples and 84~unique
attack categories, using a unified 23-metric framework spanning
accuracy, calibration, uncertainty, adversarial robustness,
computational efficiency, and privacy.

\item Integrate the developed software into a real
network-traffic-analysis and infrastructure-protection pipeline,
documenting the integration with implementation acts from
industrial-sector organisations.

\item Demonstrate that the developed software admits reproduction
in another practical area, choosing as such area the protection
of computer-vision onboard systems of unmanned aerial vehicles
with a three-tier ``onboard~-- droneport~-- cloud'' architecture;
additionally confirm domain independence by cross-domain
transfer to speech processing and medical time series.
\end{enumerate}
