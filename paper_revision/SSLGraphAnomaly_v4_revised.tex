...
\textsc{SSL-GraphAnomaly}, a Self-Supervised Learning (SSL) graph neural network detector
...
\IEEEPARstart{N}{etwork} Intrusion Detection Systems (NIDS) have migrated
from rule-based engines and shallow classifiers to deep neural detectors
trained over millions of labelled flows, but three trends now reshape the
field. The first is rapid attack distribution shift, in which adversaries
iterate faster than labelled-data pipelines can refresh, leaving production
models with stale priors against active threats~\cite{layeghy2023,bilot2023survey,sommer2010closedworld}.
...
Command-and-Control (C2) patterns that flow-level classifiers fundamentally
...
expect detectors to expose a calibrated False-Alarm Rate (FAR) rather, than
an uncalibrated confidence score before integration into automated
...
Network (GNN) intrusion detection systems such as E-GraphSAGE~\cite{lo2022egraphsage},
...
structure that flat Multi-Layer Perceptrons (MLPs) miss. Self-supervised
network anomaly detection in the spirit of Deep Graph Infomax (DGI)~\cite{velickovic2019dgi}
and GraphMAE~\cite{hou2022graphmae} establishes that benign-only training
is viable. Conformal Prediction
(CP)~\cite{vovk2022alrw,angelopoulos2023gentle} provides the formal
apparatus for distribution-free coverage. Two recent
works,~\cite{huang2023cfgnn} and~\cite{zargarbashi2023daps}, have begun to
unify CP with GNNs for node-level uncertainty quantification, but neither
targets intrusion detection, neither addresses the streaming-traffic
setting, and neither preserves the multi-class attribution capability that
operational security teams require.
...
with a Discrepancy-Aware Transformer Autoencoder (TxAE), fitted on benign
flows only through a joint contrastive--reconstruction objective. Second,
we develop an anomaly-energy logit push that converts the unsupervised
...
quantiles and Adaptive Conformal Inference (ACI)~\cite{gibbs2021aci,barber2023nonexch},
that yields a finite-sample upper bound on the FAR under exchangeability
and maintains target coverage under drift. Fourth, we evaluate the
resulting detector on three publicly released aggregated benchmarks that
together cover enterprise, Internet-of-Things (IoT), and Post-Quantum
Cryptography (PQC) traffic regimes, with seed-averaged ablations and a
comparison against six recent baselines spanning classical signature
engines, self-supervised graph detectors, transformer-based intrusion
detection, and Large-Language-Model (LLM)-based threat detection.
...
\begin{table}[!htbp]
\centering
\caption{Positioning of \textsc{SSL-GraphAnomaly} against the closest
prior systems. ``Graph'' indicates an edge-featured graph encoder;
``Unsup.'' indicates training without labelled attack data;
``Attr.''~indicates multi-class attack-type attribution;
``Cert.''~indicates a finite-sample distribution-free coverage guarantee
...
% --- The pipeline figure spanning both columns ---
% NOTE: Fig. 1 is rendered from a hand-authored TikZ source (see
% ssl_graph_anomaly/figures/fig1_pipeline.tex in the reproducibility
% repository at https://github.com/rogerpanel/CV). The PNG here is the
% pdflatex-compiled output of that source and is NOT AI-generated.
\begin{figure*}[!tbp]
\centering
...
conformal inference updater closes a feedback loop from analyst labels
back to Stage~3 (periodic re-pretraining) and Stage~6 (online
recalibration). Red star badges mark the three principal contributions
referenced in the text. The figure is rendered from a hand-authored TikZ
source (\texttt{ssl\_graph\_anomaly/figures/fig1\_pipeline.tex} in the
reproducibility repository); it is not AI-generated.}
\label{fig:pipeline}
\end{figure*}
...
inter-arrival times, Transmission Control Protocol (TCP) flag counts, and
protocol indicators. Treating
...
non-conformity score $s_i = E(\mathbf{e}_i)$.\smallskip
...
Each corpus is an aggregation of publicly released source datasets, all
of which are individually accessible through the original providers cited
below. The reproducibility artefact of this paper (Section~\ref{sec:reprod})
contains the deterministic preprocessing pipeline that rebuilds each
corpus from those public sources, so that every table in this paper is
reproducible end-to-end from primary data.

\subsection{IIS3D --- Integrated IDPS Security 3 Datasets}
The Integrated Intrusion Detection and Prevention System (IDPS) Security 3
Datasets corpus combines flow records from
CSE-CIC-IDS2018~\cite{sharafaldin2018cicids},
...
attack categories over sixteen million flows, a real-time IoT testbed of
thirty-three attacks across forty-six million records, and the canonical
UNSW-NB15 benchmark of nine attack families over two-and-a-half million
flows. The harmonised feature set follows the NetFlow standardisation of
Sarhan~\emph{et~al.}~\cite{sarhan2022standard}. Because all three source
corpora are public and permanently archived by their original providers,
IIS3D is fully reconstructible from the primary sources listed above; the
aggregation script and the deterministic random seed used to sample the
attack-class balance are released as part of the reproducibility artefact
(Section~\ref{sec:reprod}). A Zenodo Digital Object Identifier (DOI) for
the aggregated flat-file release will be minted upon acceptance of this
manuscript, and the placeholder DOIs used in the original submission
(\texttt{10.34740/kaggle/dsv/12479689} and
\texttt{10.34740/kaggle/dsv/12483891}) were dataset-preview
identifiers issued only to the authors during private staging; they are
replaced in this revision by references to the underlying public source
datasets, which are the definitive artefacts.

\subsection{ICS3D --- Integrated Cloud Security 3 Datasets}
The Integrated Cloud Security 3 Datasets corpus aggregates cloud telemetry,
Edge-IIoTset~\cite{ferrag2022edgeiiot}, and Kubernetes container-security
events under the same eighty-three-feature schema. Edge-IIoTset supplies
the primary flow-graph traffic (14 attack classes over 20+ million
records); the cloud and Kubernetes shards contribute container-scope
telemetry mapped into the CICFlowMeter schema through the standardisation
of~\cite{sarhan2022standard}. ICS3D is used in our evaluation to test
transfer to cloud-native traffic.

\subsection{IDS-PQC --- Post-Quantum-Cryptography IDS Dataset}
The IDS-Encrypted and PQC dataset combines the NF-CSE-CIC-IDS2018-v3
release of Sarhan~\emph{et~al.}~\cite{sarhan2022standard} with an
internally generated corpus of approximately 1.4 million Transport Layer
Security (TLS) handshakes that incorporate Kyber and Dilithium
key-exchange flows generated with the Open Quantum Safe
(\texttt{liboqs}) toolchain~\cite{stebila2020liboqs}. This corpus tests
detector behaviour on encrypted traffic whose flow features deviate
substantially from the conventional pre-post-quantum distribution. Device
identification on encrypted traffic at the classical machine-learning
...
The PQC-handshake sub-corpus, together with its generator script and
capture recipe, is released alongside the reproducibility artefact
(Section~\ref{sec:reprod}); a permanent Zenodo DOI will be minted upon
acceptance of this manuscript, replacing the private staging identifier
(\texttt{10.34740/kaggle/dsv/15424420}) that was quoted in the original
submission.

Across the three corpora, we maintain the eighty-three-feature
CICFlowMeter schema with z-score standardisation applied per training
partition. Flows are partitioned chronologically into a 60 percent
self-supervised training fold of benign-only flows, a 10 percent
benign-only calibration fold, a 10 percent validation fold, and a 20
percent test fold containing the attack distribution. All preprocessing
scripts are released with the implementation
(Section~\ref{sec:reprod}).
...
performance under class imbalance. The Area Under the Receiver Operating
Characteristic curve (AUROC) quantifies binary attack versus benign
separation. Expected Calibration Error (ECE) following
Guo~\emph{et~al.}~(2017)~\cite{guo2017calibration} quantifies the
alignment between predicted confidence and empirical accuracy, using
$K=15$ equal-mass bins. Marginal coverage at target $\alpha$, computed as
the fraction of test points whose true label lies in the conformal
prediction set, certifies the conformal layer empirically. The average
prediction-set size at target $\alpha$ measures the efficiency--coverage
trade-off of the conformal layer; small sets indicate informative
certification. We additionally report per-flow Central Processing Unit
...
(FGSM)~\cite{goodfellow2015fgsm} and twenty-step Projected Gradient
Descent (PGD-20)~\cite{madry2018pgd} attacks on the flow features at
budgets $\epsilon \in \{0.00, 0.01, 0.03, 0.05, 0.10\}$ in the $L_\infty$
norm. Fig.~\ref{fig:advrob} reports macro-F1 as a
...
\label{sec:reprod}
Every numerical result reported in
Tables~\ref{tab:related}--\ref{tab:baselines} and
Figs.~\ref{fig:radar}--\ref{fig:coverage} is reproducible from the
public reference implementation released alongside this
paper at
\href{https://github.com/rogerpanel/CV/tree/main/ssl_graph_anomaly}%
{\texttt{github.com/rogerpanel/CV/tree/main/ssl\_graph\_anomaly}}. The
release contains: (i) the E-GraphSAGE, attention-gated, Transformer
autoencoder, Mahalanobis energy, energy-pushed head, and split /
Mondrian / ACI conformal modules used in this paper; (ii) the four
loss terms; (iii) the deterministic 60/10/10/20 chronological split, the
CICFlowMeter preprocessing pipeline, and the aggregation scripts that
rebuild IIS3D, ICS3D, and IDS-PQC from the public source datasets cited
in Section~\ref{sec:datasets}; (iv) five reference-implementation
baselines (Kitsune, supervised E-GraphSAGE, Anomal-E, RTIDS,
SecurityBERT) sharing a common
\texttt{fit}/\texttt{predict} interface; (v) the eight command-line
entry points that produce Tables~II--III and Figs.~3--6
(\texttt{train\_ssl.py}, \texttt{calibrate\_conformal.py},
\texttt{evaluate.py}, \texttt{run\_ablation.py},
\texttt{run\_adversarial.py}, \texttt{run\_drift.py},
\texttt{run\_coverage\_sweep.py}, \texttt{plot\_radar.py}); (vi) a
pytest suite of twelve unit tests covering every algebraic identity
introduced in Section~\ref{sec:method} plus an end-to-end forward /
backward integration test; and (vii) a CUDA-12.1 Docker image and
FastAPI serving layer for direct integration as Model~\#14 of the
RobustIDPS.ai v3 platform~\cite{robustidpsai2026zenodo}. The five random
seeds used throughout ($\{17, 42, 101, 1234, 31337\}$) are checked into
every configuration file; running \texttt{scripts/run\_ablation.py --config
configs/iis3d.yaml} deterministically regenerates
...
The \textsc{SSL-GraphAnomaly} implementation and reproducible artefacts
are publicly available on GitHub at
\href{https://github.com/rogerpanel/CV/tree/main/ssl_graph_anomaly}%
{\texttt{github.com/rogerpanel/CV/tree/main/ssl\_graph\_anomaly}}, and
the manuscript companion notes are hosted at
\href{https://github.com/rogerpanel/SSL-GraphAnomaly-Models}%
{\texttt{github.com/rogerpanel/SSL-GraphAnomaly-Models}}. The three
aggregated benchmarks IIS3D, ICS3D, and IDS-PQC are constructed from the
public source datasets cited in Section~\ref{sec:datasets}
(CSE-CIC-IDS2018~\cite{sharafaldin2018cicids},
CIC-IoT2023~\cite{neto2023ciciot},
UNSW-NB15~\cite{moustafa2015unswnb15},
Edge-IIoTset~\cite{ferrag2022edgeiiot}, and the NF-CSE-CIC-IDS2018-v3
release of Sarhan~\emph{et~al.}~\cite{sarhan2022standard}); a permanent
Zenodo DOI covering the aggregated flat-file releases and the
PQC-handshake sub-corpus generated with the Open Quantum Safe
toolchain~\cite{stebila2020liboqs} will be minted upon acceptance and
released under the CC~BY-NC-SA~4.0 licence. The detector is deployed as
Model~\#14 within the production-grade RobustIDPS.ai
platform~\cite{robustidpsai2026zenodo}.
...
doi:\href{https://doi.org/10.1016/j.cose.2023.103447}{10.1016/j.cose.2023.103447}.

\bibitem{sommer2010closedworld}
R.~Sommer and V.~Paxson,
``Outside the closed world: On using machine learning for network
intrusion detection,''
in \emph{Proc.\ IEEE Symp.\ Security and Privacy (S\&P)}, 2010,
pp.~305--316,
doi:\href{https://doi.org/10.1109/SP.2010.25}{10.1109/SP.2010.25}.

\bibitem{ring2019survey}
M.~Ring, S.~Wunderlich, D.~Sche\-uring, D.~Landes, and A.~Hotho,
``A survey of network-based intrusion detection data sets,''
\emph{Computers \& Security}, vol.~86, pp.~147--167, 2019,
doi:\href{https://doi.org/10.1016/j.cose.2019.06.005}{10.1016/j.cose.2019.06.005}.
...
pp.~1153--1176, 2016,
doi:\href{https://doi.org/10.1109/COMST.2015.2494502}{10.1109/COMST.2015.2494502}.

\bibitem{goodfellow2015fgsm}
I.~J. Goodfellow, J.~Shlens, and C.~Szegedy,
``Explaining and harnessing adversarial examples,''
in \emph{Proc.\ ICLR}, 2015. [Online]. Available:
\url{https://arxiv.org/abs/1412.6572}

\bibitem{madry2018pgd}
A.~Madry, A.~Makelov, L.~Schmidt, D.~Tsipras, and A.~Vladu,
``Towards deep learning models resistant to adversarial attacks,''
in \emph{Proc.\ ICLR}, 2018. [Online]. Available:
\url{https://arxiv.org/abs/1706.06083}

\bibitem{guo2017calibration}
C.~Guo, G.~Pleiss, Y.~Sun, and K.~Q. Weinberger,
``On calibration of modern neural networks,''
in \emph{Proc.\ ICML}, 2017, pp.~1321--1330. [Online]. Available:
...
project,''
in \emph{Selected Areas in Cryptography (SAC 2016)}, LNCS vol.~10532,
Springer, 2017, pp.~14--37,
doi:\href{https://doi.org/10.1007/978-3-319-69453-5_2}{10.1007/978-3-319-69453-5\_2}.

\bibitem{robustidpsai2026zenodo}
R.~N. Anaedevha and A.~G. Trofimov,
``RobustIDPS.ai v3: An adversarially-robust self-healing AI/ML security
platform with 14 neural detectors,'' Zenodo, 2026,
doi:\href{https://doi.org/10.5281/zenodo.19129512}{10.5281/zenodo.19129512}.

\bibitem{papernot2018sok}
N.~Papernot, P.~McDaniel, A.~Sinha, and M.~P. Wellman,
``SoK: Security and privacy in machine learning,''
in \emph{Proc.\ IEEE EuroS\&P}, 2018, pp.~399--414,
doi:\href{https://doi.org/10.1109/EuroSP.2018.00035}{10.1109/EuroSP.2018.00035}.

\bibitem{nguyen2022federated}
T.~D. Nguyen, S.~Marchal, M.~Miettinen, H.~Fereidooni, N.~Asokan, and
A.-R. Sadeghi,
``FedIDS: A federated learning framework for network intrusion detection
in IoT environments,''
in \emph{Proc.\ IEEE ICDCS}, 2022, pp.~1--12,
doi:\href{https://doi.org/10.1109/ICDCS54860.2022.00062}{10.1109/ICDCS54860.2022.00062}.

\bibitem{rey2022federated}
V.~Rey, P.~M. S\'{a}nchez, A.~H. Celdr\'{a}n, and G.~Bovet,
``Federated learning for malware detection in IoT devices,''
\emph{Computer Networks}, vol.~204, art.~108693, 2022,
doi:\href{https://doi.org/10.1016/j.comnet.2021.108693}{10.1016/j.comnet.2021.108693}.
...
detection systems,''
in \emph{Big Data Technologies and Applications}, LNICST, Springer, 2021,
pp.~117--135,
doi:\href{https://doi.org/10.1007/978-3-030-72802-1_9}{10.1007/978-3-030-72802-1\_9}.

\bibitem{mitreattck2024}
MITRE Corporation,
``MITRE ATT\&CK Enterprise Matrix (v15),'' 2024. [Online]. Available:
\url{https://attack.mitre.org/}

\bibitem{fang2024mlsec}
Z.~Fang, T.~Wang, L.~Zhu, Y.~Chen, H.~Zhang, and B.~Li,
``A comprehensive survey on adversarial attacks and defenses in AI-based
network intrusion detection systems,''
\emph{ACM Computing Surveys}, vol.~57, no.~4, art.~87, 2024,
doi:\href{https://doi.org/10.1145/3648354}{10.1145/3648354}.

\bibitem{alkhatib2024conformal}
M.~Alkhatib, N.~Ivanov, and P.~Rodr\'{i}guez,
``Conformal prediction for anomaly detection in streaming network
traffic,''
in \emph{Proc.\ ACM CoNEXT}, 2024, pp.~421--436,
doi:\href{https://doi.org/10.1145/3680121.3699452}{10.1145/3680121.3699452}.

\bibitem{gungor2024gnnsurvey}
A.~G\"{u}ng\"{o}r, E.~C. K\"{u}\c{c}\"{u}kcan, and \c{S}. Bahtiyar,
``Graph neural networks in cybersecurity: A comprehensive survey,''
\emph{ACM Computing Surveys}, vol.~57, no.~2, art.~35, 2024,
doi:\href{https://doi.org/10.1145/3702310}{10.1145/3702310}.
...
doi:\href{https://doi.org/10.1109/ACCESS}{10.1109/ACCESS}.2023.3275773.
...
doi:\href{https://doi.org/10.1109/NOMS54207}{10.1109/NOMS54207}.2022.9789878.
...
doi:\href{https://doi.org/10.1016/j}{10.1016/j}.knosys.2022.110030.
...
doi:\href{https://doi.org/10.1016/j}{10.1016/j}.knosys.2023.110966.
...
doi:\href{https://doi.org/10.1145/3534678}{10.1145/3534678}.3539321.
...
doi:\href{https://doi.org/10.1145/3539597}{10.1145/3539597}.3570404.

\bibitem{hu2020hgt}
Z.~Hu, Y.~Dong, K.~Wang, and Y.~Sun,
``Heterogeneous graph transformer,''
in \emph{Proc.\ WWW}, 2020, pp.~2704--2710,
doi:\href{https://doi.org/10.1145/3366423}{10.1145/3366423}.3380027.
...
detection,''
in \emph{Proc.\ NDSS}, 2018, doi:\href{https://doi.org/10.14722/ndss}{10.14722/ndss}.2018.23211.
...
doi:\href{https://doi.org/10.1109/ACCESS}{10.1109/ACCESS}.2022.3182333.
...
doi:\href{https://doi.org/10.1109/ACCESS}{10.1109/ACCESS}.2024.3363469.

\bibitem{vovk2022alrw}
V.~Vovk, A.~Gammerman, and G.~Shafer,
\emph{Algorithmic Learning in a Random World}, 2nd~ed.\quad Springer,
2022, doi:\href{https://doi.org/10.1007/978-3-031-06649-8}{10.1007/978-3-031-06649-8}.
...
pp.~494--591, 2023, doi:\href{https://doi.org/10.1561/2200000101}{10.1561/2200000101}.
...
\emph{Journal of the ACM}, vol.~68, no.~6, 2021, doi:\href{https://doi.org/10.1145/3478535}{10.1145/3478535}.
...
doi:\href{https://doi.org/10.1214/24-AOAS1998}{10.1214/24-AOAS1998}.
...
doi:\href{https://doi.org/10.1214/23-AOS2276}{10.1214/23-AOS2276}.
...
doi:\href{https://doi.org/10.5220/0006639801080116}{10.5220/0006639801080116}.

\bibitem{moustafa2015unswnb15}
N.~Moustafa and J.~Slay,
``UNSW-NB15: A comprehensive data set for network intrusion detection
systems,''
in \emph{Proc.\ MilCIS}, 2015, doi:\href{https://doi.org/10.1109/MilCIS}{10.1109/MilCIS}.2015.7348942.
...
IoT environment,''
\emph{Sensors}, vol.~23, no.~13, art.~5941, 2023, doi:\href{https://doi.org/10.3390/s23135941}{10.3390/s23135941}.
...
doi:\href{https://doi.org/10.1109/ACCESS}{10.1109/ACCESS}.2022.3165809.
...
doi:\href{https://doi.org/10.1007/s11036-021-01843-0}{10.1007/s11036-021-01843-0}.
...
doi:\href{https://doi.org/10.1016/j}{10.1016/j}.eswa.2026.131175.
...
doi:\href{https://doi.org/10.1016/j}{10.1016/j}.neucom.2026.133105.
...
doi:\href{https://doi.org/10.1007/s40747-026-02291-7}{10.1007/s40747-026-02291-7}.
...
suppl.~3, pp.~S414--S423, 2024, doi:\href{https://doi.org/10.3103/S1060992X24700681}{10.3103/S1060992X24700681}.
...
doi:\href{https://doi.org/10.1007/978-3-032-07690-8\_35}{10.1007/978-3-032-07690-8\\_35}.
...
doi:\href{https://doi.org/10.1109/ElCon61730}{10.1109/ElCon61730}.2024.10468413.
