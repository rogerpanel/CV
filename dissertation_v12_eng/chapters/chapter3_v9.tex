...
\scriptsize (b) Live capture and classification &
\scriptsize (c) SOC Copilot (LLM assistant) \\[4pt]
...
\scriptsize (d) Adversarial robustness comparison &
\scriptsize (e) Analytics and evaluation &
\scriptsize (f) RAG Poisoning Simulator \\
\end{tabular}
\caption{Demonstration of practical operation of RobustIDPS.ai in
a typical SOC-analyst scenario: \textbf{(a)}~operator
authentication; \textbf{(b)}~Live~Monitor page~--- starting a
streaming capture on the network interface \texttt{eth0} with
selection of an ensemble of seven detection models (SurrogateIDS,
Neural~ODE, Optimal~Transport, FedGTD, SDE-TGNN, CyberSecLLM,
and others) and the capture interval (5--120~s);
\textbf{(c)}~SOC~Copilot page~--- the interactive LLM assistant
in local mode answering the analyst's query ``Get the Live
Monitor results with breakdown by threat, severity, and
confidence'' against the real data of the capture session;
\textbf{(d)}~Adversarial~Eval page~--- multi-dataset comparison
of models for robustness against six adversarial-attack methods
(FGSM, PGD, C\&W, DeepFool, Gaussian noise, Label masking);
\textbf{(e)}~Analytics \& Evaluation page~--- F1 metric
comparison for seven models across six benchmarks with a heatmap,
a radar profile, and a cross-dataset consistency table;
\textbf{(f)}~RAG Poisoning Simulator page~--- simulation of
attacks on the RAG pipeline of an LLM with four attack vectors
(document injection, query manipulation, embedding-space
perturbation, cross-contamination).}
\label{fig:gui_walkthrough}
\end{figure}
