...
(Hybrid IDPS) is in this dissertation defined as a software-hardware
defensive complex for information systems that integrates
signature-based, anomaly-based, statistical, and learning-based
components in a single defensive contour together with automatic
response facilities and interfaces to external event-logging and
response systems (SIEM/SOAR). A Hybrid IDPS differs from classical
IDS implementations in three properties:
\emph{(i)}~\emph{hybridity} of detection~--- the verdict on the
presence of an intrusion is produced by the combination of multiple
independent detectors, statistical features, and learned models
with different degrees of explainability;
...
profile), and hybrid (combination of both). By scope~--- host-based
(HIDS), network-based (NIDS), and distributed (DIDS). By deployment
topology~--- centralised, distributed, federated. By response
character~--- passive (alerting only), active (in-line
intervention), and IPS (full blocking). By stack layer~---
application, network, and physical.}
...
\subsubsection{(a) Network Traffic Protection (NIDS)}
\label{subsubsec:sector_nids}
...
\subsubsection{(b) Unmanned Aerial Vehicle Protection (UAV)}
\label{subsubsec:sector_uav}
...
\subsubsection{(c) Banking and Fintech Infrastructure Protection}
\label{subsubsec:sector_fintech}
...
data, which simultaneously demands Byzantine resilience (Model~M2)
and certified resilience to adversarial re-arrangements of the
transactional subgraph (Models~M1, M5--M6).

\subsubsection{(d) Protection of Medical Information Systems}
\label{subsubsec:sector_medical}
...
$(\varepsilon,\delta)$-differentially-private training (Model~M2)
and calibrated epistemic-uncertainty estimation (Model~M6) to
separate untrustworthy from trustworthy predictions.

\subsubsection{(e) Industrial Internet-of-Things and ICS/SCADA Protection}
\label{subsubsec:sector_iiot}
...
Hybrid IDPS is realised through Models~M4 (efficient inference
\textsf{MambaShield}) and~M1 (continuous-time telemetry
representation).

\subsubsection{(f) Autonomous Vehicle Protection}
\label{subsubsec:sector_av}
...
information-exchange topology. Hybrid IDPS in this sector uses
Models~M1, M4, and~M6 for calibrated epistemic-uncertainty
estimation as a trigger for transition to a safe-mode.

\subsubsection{(g) Cloud and SaaS Infrastructure Protection}
\label{subsubsec:sector_cloud}
...
documents the necessity of protecting this sector. Hybrid IDPS in
the cloud sector is realised through Models~M3 (multi-level
service/trace/node representations) and~M7 (game-theoretic
certification of response-strategy selection).
