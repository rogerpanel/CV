...
\chapter{Mathematical Model of Hybrid~IDPS and Proposed Models and Algorithms for Improving Its Robustness and Efficiency}
\label{ch:methods}

This chapter develops the complete theoretical core of the
dissertation. The chapter follows the structure set by the
scientific supervisor's guidance:
\S\ref{sec:hybrid_idps_math_model} formalises Hybrid IDPS as a
mathematical model and poses the six cross-cutting properties of the
system (adversarial resilience, robustness, efficiency, compatibility
with a distributed environment, the human factor, and non-stationarity)
as research questions with corresponding invariants;
\S\ref{sec:unified_problem} formulates the unified problem of
simultaneous optimisation of these properties, derived from the
fundamental graph classification problem; the next six sections
(\ref{sec:ct_tgnn}--\ref{sec:game}) present the seven developed
models and algorithms M1--M7, each of which is responsible for one
or more invariants from Table~\ref{tab:six_properties};
\S\ref{sec:metrics_taxonomy} systematises the twenty-three metrics
used to evaluate the quality of Hybrid IDPS, grouped by the six
properties. The chapter concludes with a unified certification
framework that couples all models through consistency regularisation.

The chapter is unified by three transversal mathematical primitives recurring across the seven methods. \emph{First}, the \emph{temporal-graph operator}~--- a family of mappings that act on the time-stamped tuple $(\calV_t,\calE_t,X_t,A_t)$ and that supply continuous-time integration, message passing, and selective state-space inference. \emph{Second}, the \emph{robust-optimisation min-max}~--- a family of constrained optimisation problems whose inner maximisation accounts for adversarial perturbations of features, edges, time stamps, and federated participants, and whose outer minimisation yields the parameter $\theta$ of the certified classifier. \emph{Third}, the \emph{game-theoretic equilibrium}~--- a family of zero-sum games between a defender (the Hybrid~IDPS system) and an adaptive adversary (the attacker), solved through no-regret online learning with Stackelberg-equilibrium guarantees. The seven methods M1--M7 are derived as solutions to these three primitives under increasingly demanding conditions: M1 solves the certified-dynamics primitive on continuous time; M2 solves the federated primitive on distributed participants; M3 solves the multi-scale primitive on heterogeneous graph levels; M4 solves the efficient-inference primitive at near-linear cost; M5 and M6 solve the calibration primitive on Bayesian posteriors; M7 solves the unified-certification primitive through consistency regularisation across the six preceding methods.
%%====
...
Throughout the dissertation the symbols below are used
\emph{consistently}; local redefinitions are admitted only with
explicit indication in the text.

\begin{itemize}[leftmargin=2em, itemsep=0pt]

\item $f_\theta$~--- the \emph{learnable detector} of the
Hybrid~IDPS, parameterised by $\theta\in\Theta$, mapping the
observation at time~$t$ to a probability distribution over threat
classes: $f_\theta:(X_t,A_t)\mapsto \hat y_t\in\Delta^{K}$. \emph{In
the certified architectures} (Models M1 and M3) the same
$f_\theta$ additionally serves as the right-hand side of a neural
ordinary differential equation describing the evolution of hidden
representations: $\dot{\bh}_v(t)=f_\theta(\bh_v(t),A(t),t)$; the
\emph{same} function family is implied throughout, Lipschitz
constrained with constant~$L_g$. No other function in the text
is denoted by $f_\theta$.

\item $g_\psi$~--- the \emph{response/policy mapping},
parameterised by $\psi$, mapping the posterior estimates and the
context to an action $a\in\calA$: $g_\psi:(\hat y_t,\,c_t)\mapsto
a_t$. Used in the game-theoretic block M7 and in the RL agents.
The symbol $g$ without subscript $\psi$ denotes the neural-ODE
vector field (when one needs to separate it from the detector);
this is stated locally.

\item $\pi$~--- the action-distribution \emph{policy} (in the
RL/Stackelberg sense): $\pi(a\mid s)$. Not to be confused with the
constant $\pi$ in trigonometric contexts, where the usual
interpretation is retained.

\item $L_g$~--- the \emph{Lipschitz constant} of the neural-ODE
right-hand side $f_\theta(\cdot,t)$ with respect to the hidden-state
argument: $\|f_\theta(\bh_1,t)-f_\theta(\bh_2,t)\|\le L_g
\|\bh_1-\bh_2\|$. $L_g$ is the key parameter of the Gronwall
robustness certificate: $\|\bh_1(T)-\bh_2(T)\|\le e^{L_g
T}\|\bh_1(0)-\bh_2(0)\|$. A numerical estimate $\hat L_g$ is
computed by power iteration (see Chapter~6). \emph{Defence of the
$L_g$ estimate:} the graph convolution operator is the composition
of a linear operator (the adjacency matrix), a non-linearity
(Tanh/GELU with $\mathrm{Lip}\le 1$) and a learnable weight matrix~$W$;
the upper bound $L_g\le \|W\|_{\mathrm{op}}\cdot \mathrm{Lip}(\sigma)\cdot
\|A\|_{\mathrm{op}}$ is evaluated in operator norm and tested
empirically on the validation set.

\item $L_c$~--- the \emph{Lipschitz constant of the classifier
head} $h\mapsto\mathrm{logits}$. The end-to-end Lipschitz
constant of the input-to-logits mapping is $L_c\cdot e^{L_g T}$;
the certified robustness radius in the $\ell_2$ norm is
$\rho=\Delta_{\min}/(L_c\,e^{L_g T})$, where $\Delta_{\min}$ is
the margin between the two largest logits.

\item $L_a$~--- the \emph{Lipschitz constant of the attention
mechanism} (M3, TripleE-TGNN). For softmax attention with
learnable $Q,K,V\in\R^{d\times d}$: $L_a\le
\|V\|_{\mathrm{op}}\cdot 4\,d^{1/2}\cdot \|Q\|_{\mathrm{op}}
\|K\|_{\mathrm{op}}$ (Kim et al., 2021). $L_a$ is required for
propagating certified robustness bounds through the attention
blocks: the model's end-to-end constant becomes $L_c\cdot
L_a\cdot e^{L_g T}$, reinforcing the requirement on the
spectral norm of the attention weights
$\|Q\|_{\mathrm{op}}\|K\|_{\mathrm{op}}\le 1$.

\item $\calL_g(\theta;\calG_g)$~--- the \emph{loss at granularity
level~$g$} (graph-level loss), where $g\in\{s,r,n\}$ denotes the
service / trace / node level. Here the subscript~$g$ is a
\emph{level label} and is not to be confused with the Lipschitz
constant $L_g$ or with the vector field $g$.

\item $\rho$~--- the certified \emph{robustness radius} in the
$\ell_2$ norm, or the \emph{cardinality of a set}, depending on
context (with explicit indication).

\item $\delta_X$, $\Delta_A$~--- the admissible \emph{perturbation
of node features} and the \emph{binary edge perturbation} in the
$\ell_2$ and $\ell_0$ norms respectively; they are combined in
the threat set $\Theta_{\mathrm{adv}}$.

\end{itemize}

\noindent All other symbols are introduced locally and retain
their interpretation within their own subsection.
%%====
\section{Formalisation of the Hybrid Intrusion Detection and Prevention System}
\label{sec:hybrid_idps_math_model}
%%====

\subsection{Formal Model of Hybrid IDPS}
\label{subsec:idps_formal_model}

A Hybrid IDPS as the object of research (see~\S\ref{sec:hybrid_idps_object})
is formally described by the 7-tuple
\begin{equation}
\mathcal{H} \;=\; \bigl(\mathcal{V}_t,\,\mathcal{E}_t,\,X_t,\,A_t,\,
f_\theta,\,g_\psi,\,\pi\bigr),
\label{eq:hybrid_idps_tuple}
\end{equation}where the components of the system are interpreted as follows.

\noindent\textbf{$\mathcal{V}_t$}~--- a time-evolving set of
\emph{observed entities} (graph nodes): network hosts in the NIDS
sector, onboard sensors and droneports in the UAV sector, accounts
and payment terminals in the fintech sector, medical devices and
patients in the medical sector.

\noindent\textbf{$\mathcal{E}_t$}~--- a time-evolving set of
\emph{interactions} between observed entities: network flows, command-
and-telemetry channels, transactions, accesses to electronic medical
records.

\noindent\textbf{$X_t \in \R^{n\times d}$}~--- the matrix of numerical
\emph{node features} at time~$t$, of $n=|\mathcal{V}_t|$ rows and
$d$ columns. The dimension~$d$ depends on the sector: $d=83$ for the
NIDS benchmarks (CIC-IoT-2023, CSE-CICIDS-2018), and adapts to the
number of onboard sensors and channels in the UAV sector.

\noindent\textbf{$A_t \in \R^{n\times n}$}~--- the adjacency matrix
encoding weighted \emph{links} between entities.

\noindent\textbf{$f_\theta: (X_t, A_t) \mapsto \hat{y}_t \in \Delta^K$}~---
the learnable \emph{detector} with parameters~$\theta$ that maps the
observed graph structure to a distribution over $K$ threat classes.
In this work $K=34$ for the NIDS instantiation
(Chapter~\ref{ch:adaptation}), adapted to the threat semantics of
other sectors.

\noindent\textbf{$g_\psi: \hat{y}_t \mapsto a_t \in \mathcal{A}$}~---
the \emph{response policy} with parameters~$\psi$ that selects an
action $a_t$ from a finite set~$\mathcal{A}$ (alert, flow blocking,
node isolation, transition to a safe-mode, SOAR escalation).

\noindent\textbf{$\pi \in \{\textsc{Passive}, \textsc{Active},
\textsc{IPS}\}$}~--- the \emph{integration-mode parameter} of the
defensive stack, determining the extent to which the response
policy $g_\psi$ is permitted to modify the state of the external
environment.

The Hybrid IDPS operates at discrete time steps~$t=1,2,\dots,T$
under three types of \emph{operating conditions}: the adversarial
environment, formalised by perturbations $(\delta_X, \Delta_A) \in
\mathcal{S}_\varepsilon$ bounded in norm by the radius~$\varepsilon$;
non-stationary data distribution $\mathcal{D}_t \neq \mathcal{D}_{t-1}$
with bounded drift rate; and distributed training, in which the
parameters~$\theta$ are updated by a federated algorithm over graph
fragments owned by different organisations, without raw-data
sharing.

\subsection{The Six Properties of Hybrid IDPS as Research Questions}
\label{subsec:six_properties}

The substantive work on the model~\eqref{eq:hybrid_idps_tuple} is
organised around six \emph{cross-cutting properties}, posed as
research questions in accordance with the scientific supervisor's
guidance. Each property is formalised as a mathematical condition on
the components of the tuple, and the solution to each question
within the dissertation is provided by a corresponding model or
algorithm from the list M1--M7. The summary of properties is given in
Table~\ref{tab:six_properties}.

\paragraph{(P1) Adversarial resilience (Adversarially-resilient?).}
A Hybrid IDPS is considered adversarially resilient if the detector
$f_\theta$ satisfies a \emph{quantitative sensitivity bound} on
...
\end{equation}
where $\rho(\varepsilon)\to 0$ as $\varepsilon\to 0$. This property
is covered by Models M1 (Lipschitz certificate via the Grönwall
inequality, see \S\ref{sec:ct_tgnn}), M4 (PAC-Bayesian certificates,
\S\ref{sec:mamba}), and M7 (game-theoretic certification,
\S\ref{sec:game}).

\paragraph{(P2) Robustness (Robustness?).}
A Hybrid IDPS is considered robust if, for any distribution
$\mathcal{D}_t$ that satisfies the operating-condition constraint
and for a test sample $\{(x_i,y_i)\}_{i=1}^{N}\sim\mathcal{D}_t$,
\begin{equation}
...
\end{equation}
where $\alpha$ is the operator-tunable false-alarm budget. This
property is covered by Models M1, M4, M5--M6, and M7, unified by a
single certification framework through randomised
smoothing~\cite{Cohen2019}, interval bound propagation, and
Stackelberg equilibrium against an adaptive adversary.

\paragraph{(P3) Efficiency (Efficiency?).}
A Hybrid IDPS is considered efficient if the inference time of
$f_\theta$ on a single flow does not exceed the prescribed
budget~$\tau$ with computational complexity
\begin{equation}
...
\end{equation}
where $L$ is the length of the analysed event sequence. This
property is covered by Model M4 (\textsf{MambaShield}: $O(L\log L)$
via Fast-Fourier-Transform-based convolutions, \S\ref{sec:mamba})
and the strategic choice of analysis depth in Model M7
(uncertainty-guided strategy selection).

\paragraph{(P4) Distributed environment (Distributed environment?).}
A Hybrid IDPS is considered compatible with a distributed
environment if the federated update algorithm for the
parameters~$\theta$ simultaneously satisfies an
$(\varepsilon,\delta)$-differential-privacy guarantee and is
resilient to a fraction $\beta<1/3$ of Byzantine participants:
\begin{equation}
...
\end{equation}
with simultaneous convergence of the aggregation algorithm to a
stationary point at the rate $\mathcal{O}(1/\sqrt{T})$. This
property is covered by Model M2 (\textsf{FedLLM-API},
\S\ref{sec:fedllm}).

\paragraph{(P5) Human factor (Human factor?).}A Hybrid IDPS is considered compatible with the human factor if its
outputs provide the analyst with a \emph{calibrated measure of
confidence} of expected calibration error\begin{equation}
...
\end{equation}
where $\xi$ is the admissible calibration error (in this work
$\xi=0.1$). In addition, $\hat{y}$ must admit decomposition into
epistemic and aleatoric components, allowing the analyst to triage
alerts by uncertainty. This property is covered by Models M5
(stochastic transformer) and M6 (UC-HGP, \S\ref{sec:stochastic}).

\paragraph{(P6) Non-stationarity (Non-stationarity?).}
A Hybrid IDPS is considered non-stationary-adaptive if the online
update algorithm for the parameters~$\theta_t$ achieves
\emph{sub-linear regret} relative to the best fixed model on
historical data:
\begin{equation}
...
\end{equation}
where $K$ is the size of the update-strategy set. This property is
covered by Models M1 (multi-scale time constants), M3
(\textsf{TripleE-TGNN} with elastic weight consolidation), and M7
(multi-armed bandits with game-theoretic strategy selection).

\begin{table}[H]
\centering
\caption{Summary of the six properties of Hybrid IDPS as research
questions: each property is formalised by an invariant and addressed
by one or several Models M1--M7.}
\label{tab:six_properties}
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.92}\begin{tabular}{l p{2.6cm} p{5.5cm} p{3.5cm}}
\toprule
\textbf{Label} & \textbf{Property} & \textbf{Invariant} & \textbf{Addressed by} \\
\midrule
P1 & Adv.\ resilience            & eq.~\eqref{eq:adv_resilient_property}      & M1, M4, M7 \\
P2 & Robustness                  & eq.~\eqref{eq:robustness_property}        & M1, M4, M5--M6, M7 \\
P3 & Efficiency                  & eq.~\eqref{eq:efficiency_property}        & M4, M7 \\
P4 & Distributed environment     & eq.~\eqref{eq:distributed_property}       & M2 \\
P5 & Human factor                & eq.~\eqref{eq:calibration_property}       & M5--M6 \\
P6 & Non-stationarity            & eq.~\eqref{eq:nonstationarity_property}   & M1, M3, M7 \\
\bottomrule
\end{tabular}
\end{table}

Together, the six properties P1--P6 cover the twelve intersections
of the condition--requirement matrix identified in
Chapter~\ref{ch:literature} (see Table~\ref{tab:models_to_methods}).
The substantive part of the chapter below formalises the unified
problem of simultaneous optimisation of these properties
(\S\ref{sec:unified_problem}) and develops the seven models and
algorithms M1--M7, each of which is responsible for one or more
invariants from Table~\ref{tab:six_properties}.
...
Under the adversarial environment (Condition C1), a deliberate opponent perturbs the graph. Incorporating Requirement R1 (robustness) converts the problem to a min-max formulation:\begin{equation}
\min_\theta\;
\E_{(\calG,y)\sim\calD}\!\Bigl[
...
Incorporating Requirement R2 (accuracy) on temporal graphs with hierarchical structure requires learning representations at multiple semantic and temporal scales --- component, trace, and node levels --- while preventing catastrophic forgetting as the topology evolves:\begin{equation}
\min_\theta\;
...
\label{eq:ps_triple}
\end{equation}
where $F_j$ is the Fisher information penalising departure from previously learned parameters $\theta^*$, and the KL-divergence term enforces cross-level consistency. \emph{Sub-problem 3 (Gap~3) is to solve Equation~\eqref{eq:ps_triple} with structured sparsity and quantisation-aware training. This is addressed by Model M3: TripleE-TGNN.}
...
\label{eq:ps_ssm}
\end{equation}
whose convolutional kernel is evaluated through the Fast Fourier Transform. This must be combined with progressive adversarial robustness distillation and PAC-Bayesian generalisation certificates. \emph{Sub-problem 4 (Gap~4) is to solve the efficient inference problem of Equation~\eqref{eq:ps_ssm} with formal robustness guarantees under training-time poisoning. This is addressed by Model M4: MambaShield.}
...
\emph{Sub-problem 7 (Gap~7) is to couple all methods through the consistency regularisation term $\sum_{i<j}\calL_{ij}$ in Equation~\eqref{eq:ps_unified} and prove convergence of the alternating optimisation. This is addressed by Model M7: Certification and the unified framework developed in Section~\ref{sec:unified}.}
...
%%=====================================================================
\section{Model M1: CT-TGNN --- Continuous-Time Temporal Graph Neural Dynamics}
\label{sec:ct_tgnn}
%%====
...
The continuous-time node dynamics are governed by\begin{equation}
\frac{d\bh_v(t)}{dt}
...
\end{equation}and the final embedding aggregates across scales through learned attention,\begin{equation}
\bh_v(t)
= \sum_{s=1}^{S}\beta_s(t)\;\bh_v^{(s)}(t),
\qquad
\beta_s(t) = \mathrm{Softmax}_s\!\bigl(\mathbf{w}_s^\top\bh_v^{(s)}(t)\bigr),
\label{eq:ct_tgnn_scale_fuse}
\end{equation}so that the relative contribution of each time scale adapts to the local dynamics.
...
\end{equation}Memory cost is proportional to the model size rather than the trajectory length, enabling training on arbitrarily long temporal horizons.
...
\noindent\textit{The full proof is given in Appendix~A (\S~A.1, see~\ref{app:proof_ct_conv}).}
...
\noindent\textit{The full proof is given in Appendix~A (\S~A.2, see~\ref{app:proof_ct_rob}).}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_CT-TGNN_architecture.pdf}
\caption{Architectural diagram of Model \textsf{M1: CT-TGNN}~--- the continuous-time temporal graph neural network with multi-scale ODE branches and adaptive batch normalisation. The instantiation of the model for the network-traffic-protection task, with concrete time-constant and hyperparameter values, is presented in \S\ref{sec:hyperparams} of Chapter~\ref{ch:adaptation}.}
...
\noindent\textit{The full proof is given in Appendix~A (\S~A.3, see~\ref{app:proof_fed}).}
...
To protect sample locations, Gaussian noise is added to the empirical distribution before transport computation,\begin{equation}
...
\end{equation}and Laplace noise is added to the resulting transport plan,\begin{equation}
...
\end{equation}with attention scores\begin{equation}
\alpha_{uv}^{(g,g')}(t)
...
\end{equation}while spatial attention aggregates the current neighbourhood,\begin{equation}
\hat{\bh}_v^{(g)}(t)
...
\end{equation}The combined representation is\begin{equation}
\bh_v^{(g)}(t)
= \mathrm{MLP}\!\bigl(
  [\tilde{\bh}_v^{(g)}(t)\|
   \hat{\bh}_v^{(g)}(t)\|
...
The model performs simultaneous classification at all three granularities through a multi-task loss,\begin{equation}
\calL_{\mathrm{joint}}
...
\end{equation}enforces agreement between the predictions of linked entities across abstraction levels.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_triplee_tgnn_architecture.pdf}
\caption{Architectural diagram of Model \textsf{M3: TripleE-TGNN}~--- triple-embedding multi-level temporal graph representation with dual spatio-temporal attention and elastic weight consolidation to prevent catastrophic forgetting.}
...
\end{equation}so that teacher influence grows as the poisoning intensity increases.
...
\noindent With Gaussian prior $P=\calN(\mathbf{0},\sigma_P^2\mathbf{I})$ and posterior $Q=\calN(\bm{\mu}_Q,\sigma_Q^2\mathbf{I})$, the KL term has the closed form
\begin{equation}
\mathrm{KL}(Q\|P)
= \frac{d}{2}\Bigl(
...
\end{equation}which is minimised by keeping the posterior close to the origin and its variance close to that of the prior. Full details appear in Appendix~A.\qed
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_mambashield_architecture.pdf}
\caption{Architectural diagram of Model \textsf{M4: MambaShield}~--- the selective state-space model with $\mathcal{O}(L\log L)$ inference complexity via FFT-based convolutions, augmented with progressive adversarial distillation and PAC-Bayesian generalisation certificates.}
\label{fig:m4_mambashield_arch_methods}
\end{figure}

To illustrate the operational semantics of
Algorithm~\ref{alg:mamba}, the short Python fragment below shows the
$\mathcal{O}(L\log L)$ selective-convolution kernel of
Model~\textsf{M4} via Fast Fourier Transform
(Listing~\ref{lst:mamba_fft}). The full implementation of the model
within the \textsf{robustnn-core} library is publicly available in
the \textsf{RobustIDPS.ai} open-source repository
(\url{https://github.com/rogerpanel/robustidps.ai}).

\begin{lstlisting}[language=Python,caption={$\mathcal{O}(L\log L)$
selective-convolution kernel of Model~\textsf{M4: MambaShield} via
FFT (excerpt from the Phase~A reference implementation
...
Total predictive uncertainty decomposes into epistemic uncertainty,\begin{equation}
U_{\mathrm{epi}}
= \mathrm{Var}_{p(\theta|\calD)}\!\bigl[\E_{p(y|\bx,\theta)}[y]\bigr]
\approx \frac{1}{M}\sum_{m=1}^{M}(\hat{y}^{(m)}-\bar{y})^2,
\label{eq:stoch_epi}
\end{equation}measuring the variance of the expected prediction across parameter samples, and aleatoric uncertainty,\begin{equation}
U_{\mathrm{ale}}
...
The full training loss combines five terms,\begin{equation}
\calL_{\mathrm{total}}
...
$\calL_{\mathrm{task}}=\E_{(\bx,y)}[-\log p_\theta(y|\bx)]$ is the
classification loss;
$\calL_{\mathrm{KL}}~\cite{Blundell2015}=\mathrm{KL}(q_\phi(\theta)\|p(\theta))$
regularises the variational posterior;
$\calL_{\mathrm{adv}}=\E_{(\bx,y)}\E_{\bdelta\sim q_\psi}[-\log p_\theta(y|\bx{+}\bdelta)]$
enforces adversarial robustness;
$\calL_{\mathrm{calib}}=\mathrm{ECE}~\cite{Guo2017,Naeini2015}(\{(p_\theta(\hat{y}|\bx_i),\mathbf{1}[\hat{y}=y_i])\}_i)$
penalises miscalibration; and $\calL_{\mathrm{reg}}=\|\theta\|_2^2$
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_stochastic_transformer_architecture.pdf}
\caption{Architectural diagram of Models \textsf{M5: Stochastic Transformer} and \textsf{M6: UC-HGP}~--- variational Bayesian attention (M5) with hierarchical Gaussian processes (M6) for epistemic/aleatoric uncertainty decomposition and uncertainty-guided alert triage.}
\label{fig:m5m6_stoch_trans_arch_methods}
\end{figure}

\section{Method of Adaptation to Changes in Statistical Characteristics of Network Traffic}
\label{sec:drift_adaptation}
%%====

This section defines the adaptation method for the learnable components of the Hybrid~IDPS under non-stationary network traffic. The method unifies two complementary adaptation regimes: structural protocol-regime change (Sub-problem~5: Non-stationary $\times$ Robustness, treated in subsection~\ref{sec:pq_idps}) and gradual statistical drift of feature distributions, application-profile evolution, and emergence of new operating regimes.

\subsection{Forward-Compatible Detection Under Protocol Regime Change}
\label{sec:pq_idps}
...
\subsubsection{Unified Multi-Protocol Detection Architecture}
...
\label{eq:pq_fusion}
\end{equation}and multi-task classification predicts both the category label and the target protocol,\begin{equation}
...
\subsubsection{Formal Security Reduction}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_pq_idps_architecture.pdf}
\caption{Architectural diagram of the \emph{Multi-Agent PQC-IDS module}~--- the forward-compatible intrusion-detection architecture for post-quantum protocol regime transitions, with protocol-specific encoders for lattice-, code-, and hash-based protocols and cross-attention fusion.}
\label{fig:pq_idps_arch_methods}
\end{figure}

\subsection{Adaptation to Statistical Drift of Distributions}
\label{sec:drift_adaptation_substat}
...
This section solves Sub-problem~7 by developing the Stackelberg game formulation and the consistency coupling term $\sum_{i<j}\calL_{ij}(\theta_i,\theta_j)$ from the unified problem (Equation~\eqref{eq:ps_unified}). Together with the certification components, this constitutes Model M7 and addresses Gap~7 (all conditions $\times$ all requirements).
...
\noindent\textit{The full proof is given in Appendix~A (\S~A.5, see~\ref{app:proof_regret}).}
...
\noindent\textit{The full proof is given in Appendix~A (\S~A.6, see~\ref{app:proof_unified}).}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig_game_theoretical_architecture.pdf}
\caption{Architectural diagram of Model \textsf{M7: Game-Theoretic Certification}~--- the unified framework modelling the defender--adversary interaction as a strategic game with Stackelberg equilibrium, coupling Models M1--M6 through consistency regularisation.}
\label{fig:m7_game_arch_methods}
\end{figure}

\section{Systematisation of Quality Metrics for Hybrid IDPS}
\label{sec:metrics_taxonomy}
%%====

The evaluation of the quality of the developed Models and algorithms
M1--M7 relies on a unified framework of twenty-three metrics
systematised in Table~\ref{tab:metrics_by_property} according to the
six properties of Hybrid IDPS from \S\ref{subsec:six_properties}.
Grouping the metrics by property rather than by their mathematical
origin provides a direct mapping from the experimental validation
(Chapter~\ref{ch:experiments}) onto the invariants formalised by
equations~\eqref{eq:adv_resilient_property}--\eqref{eq:nonstationarity_property}.

\begin{table}[H]
\centering
\caption{Systematisation of twenty-three quality-evaluation metrics
for Hybrid IDPS by the six properties from
...
\toprule
\textbf{Property} & \textbf{Metric class} & \textbf{Specific metrics} \\
\midrule
P1 Adv.\ resilience       & Certificates       & Certified accuracy, Grönwall radius, upper bound on attack-success rate \\
\addlinespace
P2 Robustness             & Classification     & F1 (micro/macro/weighted), balanced accuracy, MCC, Cohen's $\kappa$, AUC-ROC, TPR, TNR, FPR, FNR \\
\addlinespace
...
\addlinespace
P4 Distributed env.       & Privacy            & $(\varepsilon,\delta)$-DP budget, membership-inference advantage, Byzantine-aggregation accuracy at $\beta=0.3$ \\
\addlinespace
...
The unified treatment of the seven methods is held together by a single set of mathematical primitives recurring across the chapter: \emph{(i)~temporal-graph operators} that uniformly describe continuous-time, federated, and non-stationary settings; \emph{(ii)~Lipschitz / Gr\"onwall arguments} that propagate certified robustness from local vector fields to the global predictor; \emph{(iii)~Wasserstein / optimal-transport tools} that couple distributional alignment with the differential-privacy budget; \emph{(iv)~PAC-Bayesian and randomised-smoothing bounds} that supply distribution-free generalisation and certified-radius guarantees; and \emph{(v)~no-regret online learning} that converts adversarial-game equilibria into deployable response policies. Each of the seven methods uses at least two of these primitives, and the cross-method consistency regularisation of Method~M7 enforces that the local certificates compose into a single aggregate certificate over the seven-method ensemble. The chapter therefore delivers not only seven self-contained methods but also a coherent mathematical layer that makes the methods composable and the certificates additive.

The seven methods together realise the six framing properties P1--P6 of the Hybrid~IDPS system formulated in \S\ref{subsec:six_properties}: adversarial resilience (P1) through M1, M4, M7; robustness under non-adversarial drift (P2) through M5, M6; efficiency (P3) through M4; distributed-environment compatibility and privacy (P4) through M2; human-factor through calibrated uncertainty (P5) through M5, M6; and non-stationary adaptation (P6) through M3, M5, M6. The mapping is surjective: every property is addressed by at least one method, and every method addresses at least one property. The completeness of the mapping is the formal condition that allows the experimental chapter (Chapter~\ref{ch:experiments}) to validate the system as a whole rather than as a collection of independent components.
...
The mathematical apparatus of the chapter is presented in the order of dependency rather than in the order of historical development. This ordering choice has implications for the reader: a reader who is interested in the latest method (M7) must first understand the methods on which it depends (M1--M6), but a reader who is interested in a single method can read the corresponding section independently and rely on the proofs of that section without consulting later sections. The cross-references in the proofs are explicit about which earlier results are consumed. The chapter is therefore structured as both a sequential narrative and a reference handbook, supporting two different reading modes for two different audiences.
