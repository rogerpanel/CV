...
\paragraph{(P5) Human factor (Human factor?).}
A Hybrid IDPS is considered compatible with the human factor if its
outputs provide the analyst with a \emph{calibrated measure of
confidence} of expected calibration error
\begin{equation}
...
\begin{tabular}{l p{2.6cm} p{5.5cm} p{3.5cm}}
\toprule
...
\end{equation}
which is minimised by keeping the posterior close to the origin. \textit{The full proof is given in Appendix~A (\S~A.4, see~\ref{app:proof_pac}).}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l p{3.0cm} p{9.3cm}}
\toprule
