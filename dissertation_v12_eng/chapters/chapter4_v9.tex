...
protocol-regime migration (\S\ref{sec:mapqc_results}).
...
CIC-IoT-2023~\cite{Neto2023,Ring2019} comprises 46.6~million flow records from 105 IoT devices with 33 attack types. CSE-CICIDS2018~\cite{Sharafaldin2018} contains 16.2~million flows with 7 attack categories.
...
All methods are implemented in PyTorch~2.1~\cite{Paszke2019} with CUDA~12.1. Training uses AdamW with learning rate $2\times10^{-4}$, weight decay $10^{-5}$, and cosine annealing.
...
Overall accuracy 96.8\%, outperforming single-granularity baselines by 8.3\%. Service-level: 95.7\%, trace-level: 97.2\%, node-level: 96.4\%.
...
ECE of 0.078 versus 0.192 for deterministic attention --- a 59\% reduction. Analyst workload reduced by 43\% through uncertainty-driven triage.

\subsection{M7: Game-Theoretic Certification Results (Gap 7, Sub-problem 7)}

Accuracy 94.7\% against adaptive adversaries versus 87.6\% for non-strategic approaches. Multiplicative weights regret converges at the theoretical $\calO(\sqrt{T\log|\calC|})$ rate.
...
Every component contributes measurably. The largest single-component impact is the LLM branch ($-$53.4 F1 points), confirming its critical role in zero-shot generalisation.
...
CT-TGNN maintains 93.1\% under PGD-10 at $\epsilon=0.1$ owing to Lipschitz-bounded dynamics. SDE-TGNN certifies 76.2\% of samples within $\ell_2$ radius 0.25 via randomised smoothing.
...
Under C\&W attack across PQC migration stages, MambaShield+Structured FIM degrades by only 2.5 p.p. versus 25.5 p.p.
...
The experimental programme establishes several findings. Continuous-time dynamics provide a 6.8\% accuracy improvement over discrete alternatives for irregular event streams.
