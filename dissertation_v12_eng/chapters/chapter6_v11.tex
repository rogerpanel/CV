...
%% [REMOVED: section "Robustness Certificates for UAV Subsystems" — platform internals (now in Chapter 3)]
...
%% [REMOVED: section "Integration with the 	extsf{RobustIDPS.ai} Platform" — platform internals (now in Chapter 3)]
...
summary values are given in Table~\ref{ch:uav}.
...
Figure~\ref{ch:uav} shows robust accuracy as a function of
...
Seq2Seq Transformer~\cite{aigner_2025_uav}. Figure~\ref{ch:uav}
...
regulator (\S\ref{ch:uav}) the defining metric is the
...
Figure~\ref{ch:uav} shows mission completion as
...
The structure of the certificates produced by \S\ref{ch:uav}
...
obtained (Table~\ref{ch:uav},
Figs.~\ref{ch:uav}, \ref{ch:uav},
\ref{ch:uav}), confirming the practical
