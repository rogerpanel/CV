...
Implementation acts have been received from three organisations confirming the practical applicability of the developed AI-based models for Hybrid~IDPS in network security: NII~IKT LLC (critical-infrastructure network security, 95.7\% detection accuracy), Hexagon LLC (federated learning for commercial Hybrid~IDPS deployments and edge-computing security, 96.8\% accuracy, 40\% cost reduction), and the Artificial Intelligence Center, National Research Nuclear University MEPhI, Moscow (deployment of the developed AI-based Hybrid~IDPS framework to UAV onboard-system protection).
