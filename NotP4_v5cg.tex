...
\begin{tikzpicture}[
    ...
    servernode/.style={rectangle, draw=black!70, thick, fill=red!8, text width=3.0cm, align=center, rounded corners=2pt, minimum height=0.95cm, font=\small\bfseries},
    arrow/.style={-Stealth, thick, black!60},
    datalabel/.style={font=\tiny, black!70}
]

% Layer 1: Cloud Providers (top row, centered)
\node[cloudnode] (cloud2) {Cloud 2\\{\small (Azure)}};
\node[cloudnode, left=2.8cm of cloud2] (cloud1) {Cloud 1\\{\small (AWS)}};
\node[cloudnode, right=2.8cm of cloud2] (cloud3) {Cloud 3\\{\small (GCP)}};

% Layer 2: Feature Extraction
\node[smallblock, below=1.0cm of cloud1] (feat1) {Feature\\Extraction};
\node[smallblock, below=1.0cm of cloud2] (feat2) {Feature\\Extraction};
\node[smallblock, below=1.0cm of cloud3] (feat3) {Feature\\Extraction};

% Layer 3: Private Marginal Estimation  
\node[smallblock, below=0.9cm of feat1] (priv1) {Private\\Marginal\\Estimation};
\node[smallblock, below=0.9cm of feat2] (priv2) {Private\\Marginal\\Estimation};
\node[smallblock, below=0.9cm of feat3] (priv3) {Private\\Marginal\\Estimation};

% Layer 4: Central Server (pulled further down for clarity)
\node[servernode, below=2.0cm of priv2] (server) {Central Aggregation\\Server};

% Layer 5: Server Processing (more spread horizontally and vertically)
\node[smallblock, below=1.4cm of server] (ot) {Optimal\\Transport\\Computation};
\node[smallblock, left=3.0cm of ot] (byz) {Byzantine\\Detection};
\node[smallblock, right=3.0cm of ot] (agg) {Robust\\Aggregation};

% Layer 6: Target Deployment
\node[cloudnode, below=2.0cm of ot] (target) {Target Cloud\\Environment};
\node[smallblock, below=1.0cm of target] (detect) {Attack\\Detection};
...
% Arrows - Layer 3 to Server (labels stay clear of the server box)
\draw[arrow] (priv1) -- (server.north west) node[datalabel, pos=0.35, left=6pt] {$\tilde{\mu}_1$};
\draw[arrow] (priv2) -- (server.north)      node[datalabel, pos=0.45, right=2pt] {$\tilde{\mu}_2$};
\draw[arrow] (priv3) -- (server.north east) node[datalabel, pos=0.35, right=6pt] {$\tilde{\mu}_3$};
...
% Arrow - OT to Target
\draw[arrow] (ot) -- (target) node[datalabel, pos=0.55, right=3pt] {$T^*$};

% Arrow - Target to Detection
\draw[arrow] (target) -- (detect);

% Annotations with clearer positioning
\node[datalabel, text=red!70!black, font=\small\bfseries, right=0.25cm of priv2] {$(\epsilon,\delta)$-DP};
\node[datalabel, text=orange!80!black, font=\small, left=0.25cm of byz] {\textit{Tolerates} $q<50\%$};

% Layer labels (kept but tied to nodes so they don't drift)
\node[font=\small, text=black!60, left=1.2cm of cloud1] {Source Clouds};
\node[font=\small, text=black!60, left=1.2cm of server] {Central Server};
\node[font=\small, text=black!60, left=1.2cm of target] {Target Domain};
...
\begin{axis}[
    ...
    xmode=log,
    xmin=0.1, xmax=10,
    ymin=70, ymax=97, % increased a bit to give more headroom
    grid=major,
    % legend pos=north north, % original (commented out)
    legend style={
        at={(0.5,1.05)}, % centered above the plot, outside
        anchor=south,
        legend columns= 1,
        font=\small
    },
