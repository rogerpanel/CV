...
\author{Aminat Abiola Ajibola$^{1}$ \and \todo{co-authors}}
\date{$^{1}$\todo{Affiliation} \\[0.4em] Code, run manifests and generated tables: \url{https://github.com/rogerpanel/cv/tree/main/mlmh-appraisal}}
\begin{document}
\maketitle
\linenumbers
...
analysis unit (1{,}818 unique valid days from 162 participants); 31 statistical and circadian
...
HYPERAKTIV, where the honest estimate was at chance (0.45 to 0.46) and the leaky one reached 0.60
...
the opposite direction reduced AUROC by 0.12 to 0.20, with calibration slopes of 0.27 to 0.63 and
...
valid minutes; partial first and last days were dropped. Thirty-one predictors were computed per
...
candidate predictor (minority-class participants divided by 31) for every training cohort, which
...
(Table~\ref{tab:e1}, Figure~\ref{fig:e1}). In DEPRESJON, XGBoost rose from 0.745 (95\% CI 0.675 to
0.842) to 0.872 (0.830 to 0.918), a paired inflation of 0.114 (0.060 to 0.181); at subject level the
leaky estimate reached 0.983. In PSYKOSE, where the honest estimate is already high (0.899 to
0.906), the inflation was 0.020 to 0.044. In HYPERAKTIV the honest estimate was at or below chance
for all three models (0.447 to 0.463 window level; 0.425 to 0.445 subject level), yet record-wise
splitting produced 0.577 to 0.603 at window level and 0.601 to 0.645 at subject level, an
inflation of 0.130 to 0.169 with intervals excluding zero. Accuracy and macro-F1 behaved the same
...
(external AUROC 0.768 to 0.779 versus internal 0.716 to 0.790), because schizophrenia produces the
larger activity deficit. Models trained on PSYKOSE lost 0.12 to 0.20 AUROC when applied to
DEPRESJON (logistic regression 0.881 $\rightarrow$ 0.689; XGBoost 0.871 $\rightarrow$ 0.670), and their
...
\todo{Two sentences from results/real/E5/e5\_obf.csv once the run completes: any-psychiatric-vs-control AUROC subject-wise vs record-wise; five-class macro-AUROC subject-wise vs record-wise.}

\subsection{Supplementary arm: 1D-CNN on the raw series}
\input{tables/e1_auroc_inflation_e1_cnn}
\todo{One sentence from results/real/E1\_cnn/e1\_results.csv once the run completes.}
...
useful classifier. Skipping external validation hid a loss of up to 0.20 AUROC and a collapse of
...
The honest subject-level AUROC of 0.86 to 0.89 on DEPRESJON is consistent with the dataset
authors' leave-one-patient-out baseline \cite{garciaceja2018} and well below the 0.95 to 0.98
...
S1 TRIPOD+AI checklist (\texttt{supplement\_tripod\_checklist.tex}); S2 feature definitions
(\texttt{src/mlmh/features/actigraphy.py}); S3 per-window predictions and run manifests
(\texttt{results/real/}).
