...
\date{$^{1}$\todo{Affiliation, University of Hafr Al-Batin} \\[0.5em] Protocol registered on PROSPERO: \todo{CRD number}}
\begin{document}
\maketitle
\linenumbers
...
cross-validation inflated AUROC by 0.04 to 0.17, external validation reduced AUROC by up to 0.20
...
(AUROC 0.45 to 0.46): leakage did not merely exaggerate a real signal, it manufactured one.
Applying a model trained on one cohort to another without refitting reduced AUROC by 0.12 to 0.20
