...
$\rho\le 1$. We further treat the HGT attention coefficients
$\alpha_{u,v,r}$ as bounded by their spectral envelope, so the
state-dependent softmax is upper-bounded by the relation-weight
spectral norms; a tighter, state-aware bound is left to future
work. Let the per-node temporal-neighbourhood degree be bounded
by $d_{\max}$, and let $S$ be the client-update clipping threshold
used at Algorithm~\ref{alg:fedloraguard} line~5.
...
FLTrust asks the coordinator to hold a small vetted root set $R_0$
of $200$ clean adapters. This is not a paradox with the paper's
...
the recurring, per-upload verification of the full marketplace
stream (hundreds of thousands of adapters, continuous ingestion),
which manual inspection cannot cover. Bootstrapping trust from a
small clean set to score an unbounded dynamic stream is the
standard FLTrust operating model and does not require the operator
to manually vet every upload. The residual assumption is that
$R_0$ is and remains clean; rotating $R_0$ periodically and
attesting its integrity through the marketplace's own signing
pipeline is a straightforward operational mitigation.
