...
The verified threats --- alignment-bypass fine-tunes, composite
textual-trigger backdoors and instruction-following corruption ---
are attacks on the safety of NLP systems at the PEFT layer, the
multimodal encoder consumes model-card and contributor text, and
\benchname\ extends NLP-safety resources; this is why we position
the work for the ACL community rather than a general security
venue. Sections~\ref{sec:related}--\ref{sec:conclusion} present
related work, the ecosystem formalisation, the framework, the two
theorems, the empirical study, and the discussion.
...
\begin{proof}[Sketch]
Three ingredients (details in App.~\ref{app:proofs}). (i)~PixelDP
Lemma~1~\cite{lecuyer2019certified}: an $(\epsilon_r,\delta)$-DP
scoring function $f$ has
$\Prob[\argmax f(\mathbf{x})\!\neq\!\argmax f(\mathbf{x}+\Delta)]\!\le\!
e^{\epsilon_r} p_\Delta(\mathbf{x})+\delta$; the smoothed random
variable is the aggregated global model $\mathbf{w}_T$.
(ii)~Under Def.~\ref{def:attack}, one client contributes an
$\ell_2$-perturbation of size $\!\le\!\epsilon_{\mathrm{wt}}$
per round after clipping, so $k$ colluding clients contribute at
most $k\epsilon_{\mathrm{wt}}$ (triangle); FLTrust $\beta_i\!\in\!
[0,1]$ can only tighten, so any unweighted bound holds under
FLTrust. (iii)~CRFL~\cite{xie2021crfl} Thm.~1 converts a
class-margin radius into a client coalition of size $N$; combining
with the PixelDP stability multiplier $e^{\epsilon_r}$ and the CRFL
denominator $2$ gives Eq.~\eqref{eq:kstar}. R\'enyi-DP
composition~\cite{mironov2017renyi,mironov2019sampled} produces
$\epsilon_T$ but is orthogonal to the certificate. Failure
probability $T\delta$ is a union bound over inference draws.
\end{proof}
